...
# size.int<-read.csv('size_scale_integration_noApods_18_08_2022.csv')

size.int<-size.int[,-1]
size.int[size.int < 0 ]<-0
size.int.p<-read.csv('size_scale_integration_p_11_08_2022.csv')

# size.int.p<-read.csv('size_scale_integration_p_noApods_18_08_2022.csv')
...
geom_text(x=2.0, y=5.25, aes(label=c('Wing')), size=6 )+
lims(y=c(2.5,5.5))+ 
labs(colour='module',linetype='module')+
...
ggplot(longData, aes(x = Var2, y = Var1)) + 
  ...
  theme_bw() + theme(axis.text.x=element_text(size=8, angle=90, vjust=0.3,colour=rev(bone_colours),face = "bold"),
				...
				legend.key.height=unit(1,'cm'), # was 1
...
# Let's find a way to arrange these plots to leave a gap for an illustration,
...
dev.new(height=12,width=18,unit='cm')
...
# try setting aspect ratios of other plots before plotting?
# try reducing vertical size of delta Z scale bar

# ggsave(filename='mass_v_integration_14_11_2022b.pdf')
# You may choose to save the plot. 


# ggsave(filename='mass_v_integration_nApods_18_08_2022b.pdf')
# 